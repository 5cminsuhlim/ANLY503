...
```{python}
## modified
df.loc[df['hospital_name']=='Tin Shui Wai   (A&E service from 8:00am to 8:00pm)', 'hospital_name'] = 'Tin Shui Wai' # merge duplicate

(alt.Chart(df)
 .mark_rect()
 .encode(x=alt.X('yearmonthdate(hospital_time):O', title="Date"),
         y=alt.Y('hospital_name:N', title='Hospital Name'),
         color=alt.Color('mean(max_wait):Q',
                         scale=alt.Scale(scheme='orangered'),
                         legend=alt.Legend(type='symbol'),
...
```{python}
## modified
# same "selection" variable across all three plots
...
line1=(alt.Chart(df)
 ...
 .encode(x=alt.X('hours(hospital_time):T'),
         y='mean(max_wait):Q',
         color=color2,
         opacity=alt.condition(selection, alt.value(1), alt.value(0.2)) # change opacity based on selection
        )
 .add_selection(selection)
)
...
).transform_filter(selection) # transform plot shown based on selection
