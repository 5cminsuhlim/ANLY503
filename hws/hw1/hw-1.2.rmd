...
g <- ggplot(df, aes(mentor, articles, color = kids)) + 
  ...
  theme(axis.title.x = element_text(size=14), # axes titles
...
g <- ggplot(gapminder, aes(x=year, y=pop)) +
  geom_line(aes(col=continent, group=country), lwd=0.7) + # regression line
  ...
  theme(panel.background = element_rect(fill='white'), # panel styling
        ...
        strip.background = element_rect(fill='grey', color='black', linewidth = 1)) + # outer box around continents
